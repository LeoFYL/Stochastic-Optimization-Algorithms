...
while (minimumPathLength > targetPathLength)
 ...
 for k = 1:numberOfAnts
  ...
  if (pathLength < minimumPathLength)
    ...
    bestPath = path; % Store the best path found
 ...
 fid = fopen('BestResultFound.m', 'w'); % Open file for writing
fprintf(fid, 'bestPath = [');
fprintf(fid, '%d ', bestPath(1:end-1)); % Write city indices separated by spaces
fprintf(fid, '%d];', bestPath(end)); % Write the last city index and close the vector with ];
